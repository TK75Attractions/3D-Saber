...
public static class JudgmentTierHelper
{
    ...
    // 比率を維持したまま圧縮（Bad=270ms）：
    //   Perfect=90ms (33.3%)、Great=150ms (55.6%)、Good=210ms (77.8%)、Bad=270ms (100%)
    public static JudgmentTier Classify(double errorSeconds)
    {
        ...
        if (abs <= 0.15) return JudgmentTier.Great;
        if (abs <= 0.21) return JudgmentTier.Good;
        if (abs <= 0.27) return JudgmentTier.Bad;
